...
^|I |The links array property SHALL contain **only one** link with a link relation value of `+canonical+`, `+update+`, or `+deletion+`.  These link relations SHALL NOT be combined within a single notification. The `href` property for the link relation value of `canonical` or `update` SHALL be unique.
|===
//req11
